...
export default function TablaItems({ 
  ...
  const handleAjustar = async (carritoId, delta) => {
    ...
    // Si ya está procesando este botón, ignorar
    ...
    try {
      ...
      // Quitar el estado de procesando después de 500ms
      setTimeout(() => {
        setProcesando(prev => {
          const nuevo = { ...prev };
          delete nuevo[key];
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Tipo de item (si es piso o producto) */}
                  {item.tipo && (
                    <span className="text-[9px] md:text-[10px] text-gray-400 font-semibold uppercase">
                      {item.tipo}
                    </span>
                  )}
                </div>
