    ...
    <div className="min-h-screen bg-gray-50 text-black">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <form onSubmit={buscarPedidos} className="max-w-md mx-auto mb-8">
          <div className="flex gap-2">
            <input
              ...
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 text-black placeholder:text-black"
              required
            />
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2 bg-yellow-400 text-black font-bold rounded-lg hover:bg-yellow-500 disabled:opacity-50"
            >
              {loading ? 'Buscando...' : 'Buscar'}
            </button>
          </div>
        ...
        {buscado && !loading && (
          <div className="space-y-4">
            {pedidos.length === 0 ? (
              <p className="text-center text-black">No se encontraron pedidos</p>
            ) : (
              pedidos.map((pedido) => {
                const isEntregado = pedido.estatus_pedido === 'Entregado';
                return (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        <p className="font-bold text-black">Pedido #{pedido.id}</p>
