...
//                   <div className="mb-4">
//                     <label className="block text-sm font-medium mb-1 text-gray-800">Código postal de entrega *</label>
//                     <input
//                       type="text"
...
//                     <span className="font-bold text-black">${gastosManiobra.toFixed(2)}</span>
//                   </div>
//                 </div>
...
//                               <option key={est} value={est} className="text-black">
//                                 {est}
//                               </option>
//                             ))}
//                           </select>
...
export default function Checkout() {
  ...
  const confirmarPago = async () => {
    ...
    try {
      ...
      // 2. Procesar pago (vaciar carrito y descontar stock)
      const vaciarRes = await fetch('/api/carrito/vaciar', {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (!vaciarRes.ok) {
        // Si hay error de stock, mostrar mensaje detallado
        if (vaciarData.error === 'Stock insuficiente') {
          toast.error(
            `Stock insuficiente para ${vaciarData.item}. Disponible: ${vaciarData.disponible}, solicitado: ${vaciarData.solicitado}`,
            { duration: 5000 }
          );
          // Refrescar carrito para actualizar stocks
          await fetchCarrito();
          setProcesando(false);
          return;
        } else {
