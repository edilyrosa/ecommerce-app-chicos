...
export default function CrudBanners() {
  ...
  // --- useEffect de autenticación y redirección ---
  useEffect(() => {
    if (!authLoading) {
  ...
  return (
    // Aseguramos que todo el texto dentro de este div herede color negro
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-8 text-black">
      <Header />
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <h1 className="text-3xl font-black mb-8 text-center text-black">
          Administrar Banners
        </h1>
        ...
        {uploading && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="bg-white rounded-xl p-6 shadow-2xl flex flex-col items-center gap-3">
              ...
              <p className="font-bold text-black">Subiendo imágenes...</p>
            </div>
          </div>
        )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 border rounded-lg bg-gray-100 text-black"
                  disabled
                />
              </div>

              <div>
                <label className="block text-sm font-bold mb-1 text-black">Título *</label>
                <input
                  type="text"
                  name="titulo"
                  value={form.titulo}
                  onChange={handleChange}
                  className="w-full p-2 border rounded-lg text-black placeholder:text-black"
                  required
                />
              </div>

              <div>
                <label className="block text-sm font-bold mb-1 text-black">Descripción</label>
                <textarea
                  name="descripcion"
              ...
              {form.tipo === 'categorias' ? (
                <>
                  <div>
                    <label className="block text-sm font-bold mb-1 text-black">Categoría</label>
                    <input
                      type="text"
               ...
               <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-xs font-bold text-blue-800 mb-1">📐 Recomendación de dimensiones (basado en el diseño responsivo)</p>
                  <ul className="text-[11px] text-blue-700 space-y-1 list-disc list-inside">
                    <li><strong>Imagen izquierda (principal):</strong> <strong>1920 × 1080 px</strong> (relación de aspecto de 1.73:1 (casi 16:9) — se adapta perfectamente al slot de ~725×420px)</li>
                    <li><strong>Imágenes derechas (dos iguales):</strong> <strong>1400 × 600 px</strong> (relación de aspecto de  2.34:1 (casi 7:3) — ideal para el slot panorámico de ~483×206px)</li>
                    <li>Usa imágenes de al menos <strong>1920 px de ancho</strong> para la izquierda y <strong>1400 px</strong> para las derechas, garantizando nitidez en todos los dispositivos.</li>
                    <li>Formatos recomendados: <strong>JPEG</strong> (para fotos) o <strong>WebP</strong> (mejor compresión).</li>
                  </ul>
                  <p className="text-[10px] text-blue-600 mt-2 italic">Estas dimensiones optimizan la carga y la calidad visual en móviles, tablets y escritorio.</p>
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="bg-gray-300 text-black font-bold py-2 px-6 rounded-lg hover:bg-gray-400 transition"
                >
                  Cancelar
                </button>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          <div className="mb-4 border rounded-lg overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 transition font-bold text-left text-black"
            >
              <span>Categorías</span>
              {openSection === 'categorias' ? <ChevronDown size={20} className="text-black" /> : <ChevronRight size={20} className="text-black" />}
            </button>
            {openSection === 'categorias' && (
              <div className="p-4 space-y-2">
                {Object.keys(bannersPorCategoria).length === 0 ? (
                  <p className="text-black italic">No hay banners de categorías.</p>
                ) : (
                  Object.keys(bannersPorCategoria).map((cat) => (
                    <div key={cat} className="border rounded-lg overflow-hidden">
                      <button
                        onClick={() => toggleCategoriaSub(cat)}
                        className="w-full flex items-center justify-between p-3 bg-white hover:bg-gray-50 transition text-black"
                      >
                        <span className="font-medium text-black">{cat}</span>
                        {openCategoriaSub === cat ? <ChevronDown size={16} className="text-black" /> : <ChevronRight size={16} className="text-black" />}
                      </button>
                        ...
                        <div className="p-3 space-y-2 bg-gray-50">
                          {bannersPorCategoria[cat].map((banner) => (
                            <div key={banner.id} className="border rounded-lg p-3 flex justify-between items-center bg-white">
                              <div>
                                <p className="font-bold text-black">{banner.titulo || 'Sin título'}</p>
          ...
          <div className="mb-4 border rounded-lg overflow-hidden">
            ...
            >
              ...
              {openSection === 'promocion' ? <ChevronDown size={20} className="text-black" /> : <ChevronRight size={20} className="text-black" />}
            </button>
            {openSection === 'promocion' && (
              <div className="p-4 space-y-2">
                {bannersPorTipo.promocion.length === 0 ? (
                  <p className="text-black italic">No hay banners de promociones.</p>
                ) : (
          ...
          <div className="border rounded-lg overflow-hidden">
            ...
            >
              ...
              {openSection === 'productos nuevos' ? <ChevronDown size={20} className="text-black" /> : <ChevronRight size={20} className="text-black" />}
            </button>
            {openSection === 'productos nuevos' && (
              <div className="p-4 space-y-2">
                {bannersPorTipo['productos nuevos'].length === 0 ? (
                  <p className="text-black italic">No hay banners de productos nuevos.</p>
                ) : (
